# program created using Michael's code to run dust detection on each image
...
path="/home/large_data/southpole_data/dmlab/solar/south-pole/K-0/2017/1/21/13/"
...
img_path= '/home/large_data/venus_work/bounding_box3/'
os.chdir(img_path)
target_class = 2
for z,image_dup in enumerate(imgs):
    ...
    for index in range(0, len(k_obj.spot_dict)-1): 
        ...
        #plt.savefig(img_path + "/boxed_img_%03d.png" % z, dpi = 600, pad_inches = 0 , bbox_inches='tight')
        cv2.imwrite("boxed_img_%03d.png" % z, copy_copy)
        plt.draw()
        plt.close()